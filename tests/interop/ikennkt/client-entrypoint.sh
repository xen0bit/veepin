#!/bin/sh
# ikennkt VPN client (ikev2) container entrypoint for the interop harness.
...
echo "ikennkt-client: connecting to $SERVER:${PORT:-500} as $CLIENT_ID (server-id=$SERVER_ID)"
...
while [ "$i" -le 30 ]; do
    ikev2 \
        ...
        -tun tun0 \
        -full-tunnel=false
    echo "ikennkt-client: attempt $i failed; retrying in 2s"
    i=$((i + 1))
    sleep 2
done

echo "ikennkt-client: giving up after $((i - 1)) attempts"
exit 1
