#!/bin/sh
# veepin Ivanti Connect Secure gateway for the interop harness.
#
...
# off). NO_ESP=1 leaves the UDP port unbound, serving the IF-T/TLS path only.
set -eu
SHAPE="${SHAPE:-0}"
...
if [ "${NO_ESP:-0}" = "1" ]; then
    ESP_FLAG="-no-esp"
    echo "veepin-pulse-server: serving the IF-T/TLS data path only"
fi
