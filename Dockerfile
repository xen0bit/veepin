...
#
# The binary is pure-Go and CGO-free, so the build is fully static and the
# runtime image only needs the userspace networking tools it shells out to
# (ip/iptables/sysctl) plus ping for the data-path assertion. TUN access
...
# copy the source and build the command into /out.
COPY . .
