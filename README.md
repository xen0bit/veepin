...
The matrix below is regenerated by CI from the live interop run on every push to
main — each ✓ is a Docker test that passed in that run, not a claim.
...
_The throughput table is generated by CI from the interop run; it fills in on the first push to main._
...
_The full benchmark table is generated by CI; run `go test -bench=. -benchmem ./...` locally to preview it._
