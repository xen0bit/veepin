...
**Goal.** A NetworkManager VPN type per veepin protocol — "veepin IKEv2",
"veepin WireGuard", and so on — each of which:

- appears alongside OpenVPN / WireGuard / PPTP in the desktop VPN UI as its own
  entry, so choosing a veepin protocol is one pick in the *Add VPN* list rather
  than a second dialog, and is toggleable from the GNOME quick-settings VPN menu;
...
└── nm/                       # NESTED MODULE — may use deps + CGO/C
    ...
    ├── data/
    │   ├── nm-veepin-service.name.in       # NM VPN descriptor template,
    │   │                                   #   expanded once per protocol
    │   └── nm-veepin-service.conf          # D-Bus system policy (own_prefix)
...
### 5.1 `.name` descriptors (`nm/data/nm-veepin-service.name.in`)

**One VPN type per protocol.** NetworkManager builds the OS's *Add VPN* list
from one entry per descriptor in `/usr/lib/NetworkManager/VPN/`, so veepin ships
ten — `nm-veepin-ikev2-service.name` and so on — and every veepin protocol
appears in that list directly. There is no umbrella "veepin" entry that opens a
second dialog to choose a protocol; that is what this replaced.

The descriptors are generated by the Makefile from one template and one list
(`VEEPIN_PROTOCOLS`), so adding a protocol is a line in that list plus its field
table in `veepin-editor.c`:

```ini
[VPN Connection]
name=veepin IKEv2
service=org.freedesktop.NetworkManager.veepin.ikev2
program=/usr/lib/NetworkManager/nm-veepin-service
supports-multiple-connections=false

[libnm]
plugin=libnm-vpn-plugin-veepin-ikev2.so

[GNOME]
auth-dialog=/usr/lib/NetworkManager/nm-veepin-auth-dialog
properties=libnm-vpn-plugin-veepin-ikev2.so
supports-external-ui-mode=false
```

Three couplings this file cannot enforce on its own, each checked by
`editor_smoketest`:

- **`service=` must equal what the named `.so` reports.** libnm refuses to load
  a plugin whose `NM_VPN_EDITOR_PLUGIN_SERVICE` differs, and the failure mode is
  silent: the VPN type simply does not appear in the GUI. This is also why there
  is one `.so` per protocol rather than one shared library — the factory entry
  point `nm_vpn_editor_plugin_factory()` takes no service argument, so a library
  can answer to exactly one service name.
...
- Requests the bus name NetworkManager passed as `--bus-name` — one of
  `org.freedesktop.NetworkManager.veepin.<protocol>` — on the **system** bus;
  exports the `VPN.Plugin` object. Honouring the flag is what lets one binary
  back every VPN type; claiming a fixed name instead would leave NM waiting
  forever on all but one protocol.
...
The GTK **editor** (`nm/editor`) is compiled once per protocol — one `.so` each,
with `-DVEEPIN_PROTOCOL="ikev2"` and so on — so the form a user gets is just that
...
Each editor still writes `protocol=<id>` into `vpn.data` even though the service
name already says which protocol it is. That keeps the Go side's parsing
identical however a connection was created — GUI, `nmcli`, or a hand-written
keyfile:

```sh
nmcli connection add type vpn \
  vpn-type org.freedesktop.NetworkManager.veepin.wireguard \
  con-name wg-home \
...
names and NM to call them. `own_prefix` rather than `own`, because there is one
name per protocol — so adding a protocol stays a change to the Makefile's list
alone. The stock openvpn and pptp policies do the same for the same reason:
...
<busconfig>
  <policy user="root">
    <allow own_prefix="org.freedesktop.NetworkManager.veepin"/>
    <allow send_destination="org.freedesktop.NetworkManager.veepin"/>
  </policy>
  <policy context="default">
    <deny own_prefix="org.freedesktop.NetworkManager.veepin"/>
    <allow send_destination="org.freedesktop.NetworkManager.veepin"/>
  </policy>
</busconfig>
```
...
nmcli connection add type vpn con-name home-veepin ifname '*' \
  ...
  vpn.data 'protocol=ikev2, gateway=vpn.example.com, local-id=client.example.com, server-id=vpn.example.com, full-tunnel=yes'
...
```sh
# PSK. The vpn-type is the protocol's own service; protocol= should match it.
nmcli connection add type vpn con-name home-veepin ifname '*' \
  vpn-type org.freedesktop.NetworkManager.veepin.ikev2 \
  vpn.data 'protocol=ikev2, gateway=vpn.example.com, local-id=client.example.com, server-id=vpn.example.com, full-tunnel=yes'
nmcli connection modify home-veepin vpn.secrets 'psk=a-strong-preshared-key'
...
```sh
# -bus-name is what NetworkManager passes to select the VPN type; without it the
# service claims the bare prefix, which backs no type.
nm/bin/nm-veepin-service -session -bus-name org.freedesktop.NetworkManager.veepin.ikev2 &
busctl --user introspect org.freedesktop.NetworkManager.veepin.ikev2 \
